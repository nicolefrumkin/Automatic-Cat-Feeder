...
int readPotentiometer() {
    ...
    // Map to portion range (30-75 grams)
    int portionGrams = map(rawValue, 0, 4095, MIN_PORTION, MAX_PORTION);
...
bool validateSensorReadings() {
    ...
    // Buttons should be HIGH when not pressed (pullup resistors)
    if (buttonState != HIGH && buttonState != LOW) {
        Serial.println("ERROR: Button pin floating");
        allSensorsValid = false;
    } else {
    ...
    if (switchState != HIGH && switchState != LOW) {
        Serial.println("ERROR: Switch pin floating");
        allSensorsValid = false;
    } else {
        Serial.println("Switch pin: OK");
    }
    
    // Test 3: Servo attachment
    if (!servoAttached) {
        Serial.println("ERROR: Servo not attached");
        allSensorsValid = false;
        ...
        if (feederServo.attach(SERVO_PIN, SERVO_MIN_PULSE, SERVO_MAX_PULSE)) {
            ...
            Serial.println("Failed to reattach servo");
            allSensorsValid = false;
        }
    } else {
    ...
    // Test 5: HX711 pins (basic connectivity test)
    pinMode(HX711_BOWL_SCK, OUTPUT);
    pinMode(HX711_TANK_SCK, OUTPUT);
    digitalWrite(HX711_BOWL_SCK, LOW);
    digitalWrite(HX711_TANK_SCK, LOW);
    ...
    bool hx711Bowl = digitalRead(HX711_BOWL_DT);
    bool hx711Tank = digitalRead(HX711_TANK_DT);
    
    // Data pins should be readable (not floating)
    Serial.print("HX711 Bowl DT: ");
    Serial.println(hx711Bowl ? "HIGH" : "LOW");
    Serial.print("HX711 Tank DT: ");
    Serial.println(hx711Tank ? "HIGH" : "LOW");
    
    // Update last valid reading timestamp
    if (allSensorsValid) {
        lastValidSensorReading = millis();
